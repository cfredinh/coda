...
def model_to_CPU_state(net):
    ...
    if is_parallel(net):
        if isinstance(net, dict):
            state_dict = {m:{k: deepcopy(v.cpu()) for k, v in n.module.state_dict().items()}
                          for m, n in net.items()}
        else:
            state_dict = {k: deepcopy(v.cpu()) for k, v in net.module.state_dict().items()}
    else:
        if isinstance(net, dict):
            state_dict = {m:{k: deepcopy(v.cpu()) for k, v in n.state_dict().items()}
                          for m, n in net.items()}
        else:
            state_dict = {k: deepcopy(v.cpu()) for k, v in net.state_dict().items()}
            
    if isinstance(net, dict):
        state = {m:OrderedDict(s) for m, s in state_dict.items()}
    else:
...
class RandomIntensityBrightness(object):


    def __init__(self, p=0.5, intensity=0.005, brightness=0.005):
        self.prob = p
    ...
    def __call__(self, sample):
        ...
        # intensity shift
        image = torch.clip((image+torch.empty(3,1,1).normal_(mean=0.0,std=self.intensity)), 0.0, 1.0)

        # brightness shift
        image = torch.clip(pow(image, torch.clip(torch.empty(3,1,1).normal_(mean=1.0,std=self.brightness),1.0-self.brightness,1.0+self.brightness)),0.0,1.0)
...
class SelfNormalize(object):
    """Normalize the input along channel axis based on the input image's mean and std

    Input:

        Image sample, torch tensor
    """

    def __init__(self):
        print("Using Self Normalization")

    def __call__(self, image):
        
        
        mean_1, std_1 = image.mean([1, 2], keepdim=True), image.std([1, 2], keepdim=True)

        image = (image - mean_1) / (std_1 + 1e-6)
...
def create_umap_embeddings(feature_bank, targets, label_mapper=None, umap_path="umap_emb"):
    ...
    df_umap = pd.DataFrame({"x": embedding.embedding_[:, 0], "y": embedding.embedding_[:, 1], "label":named_targets})
    df_umap.to_csv(umap_path[:-4]+".csv")
    df_umap = pd.DataFrame(feature_bank)
    df_umap["label"] = targets
    df_umap.to_csv(umap_path[:-4]+"-features.csv")
