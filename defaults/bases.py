...
class BaseSet(Dataset):
    ...
    def get_trans_list(self, transform_dict):
        ...
        if "RandomIntensityBrightness" in transform_dict:                
            if transform_dict['RandomIntensityBrightness']['apply']:
                transform_list.append(
                    RandomIntensityBrightness(p=transform_dict['RandomIntensityBrightness']['p'],
                                       intensity=transform_dict['RandomIntensityBrightness']['intensity'],
                                       brightness=transform_dict['RandomIntensityBrightness']['brightness'])) 
        ...
        if "SelfNormalize" in transform_dict:            
            if transform_dict['SelfNormalize']:
    ...
    def get_transform_defs(self):
        if self.mode == 'train':
            applied_transforms = self.train_transforms
        if self.mode in ['val', 'eval']:
            applied_transforms = self.val_transforms
        if self.mode == 'test':
            applied_transforms = self.test_transforms
        return applied_transforms
    
    def has_multi_crop(self):        
        applied_transforms = self.get_transform_defs()
        
        if "MultiCrop" in applied_transforms:
            self.is_multi_crop = applied_transforms["MultiCrop"]["apply"] 
            self.multi_crop_aug = MultiCrop(n_crops=applied_transforms["MultiCrop"]["n_crops"],
                                                    sizes=applied_transforms["MultiCrop"]["sizes"],
                                                    scales=applied_transforms["MultiCrop"]["scales"])
        else:
            self.is_multi_crop = False
    ...
    def get_transforms(self):   
        ...
        if self.is_multi_crop and "RandomResizedCrop" in applied_transforms:
            applied_transforms["RandomResizedCrop"]["apply"] = False # turning off RandomResize in augs
...
class BaseModel(nn.Module):
    ...
    def is_cnn(self, backbone_type):
        if hasattr(cnn_models, backbone_type):
            return True
        return False
    
    def is_vit(self, backbone_type):
        if hasattr(transformers, backbone_type):
            return True
        return False               
...
class BaseTrainer:
    ...
    def set_models_precision(self, model=None, use=False):
